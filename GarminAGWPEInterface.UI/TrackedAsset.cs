...
namespace GarminAGWPEInterface.UI
{
    public class PositionType
    {
        ...
        public PositionType(byte[] buffer, ref int offset)
        {
            Latitude = BitConverter.ToInt32(buffer, offset) * 180.0 / int.MaxValue;
            offset += 4;
            Longitude = BitConverter.ToInt32(buffer, offset) * 180.0 / int.MaxValue;
            offset += 4;
        }
    ...
    public class TrackedAsset
    {
        ...
        public ushort CollarId;
        public byte Color;
        public byte Battery;
        public byte Gps;
        public byte Comm;
        public byte Channel;
        public byte SquelchCode;
        public byte Index;
		...
		public override string ToString()
		{
            return $"{this.Identifier,-20}Time:{this.Time:T}     Batt:{this.Battery}/4     Comm:{this.Comm}/5     GPS:{this.Gps}/3     ID:{this.CollarId / 256}-{this.CollarId % 256}     {this.Position.Latitude:0.000000}, {this.Position.Longitude:0.000000}     {this.DogStatus}";
        }

        public String ToAPRSString(String callSign)
		{
            String fromCallSign = "K9BDM"; // "G1";
            String fromSSID = "11"; // this.Identifier.ToUpper();
            String toCallSign = "APRS";
            Double latitude = this.Position.Latitude;
            Double longitude = this.Position.Latitude;
            Double altitude = this.Altitude;
            String symbolTable = "/";
            String symbol = "p";
            Int32 latitudeDegress = (Int32)Math.Abs(latitude);
            Double latitudeMinutes = (Math.Abs(latitude) - latitudeDegress) * 60;
            String latitudeDirection = (latitude < 0) ? "S" : "N";
            String latitudeDDM = $"{latitudeDegress:00}{latitudeMinutes:00.00}{latitudeDirection}";
            Int32 longitudeDegress = (Int32)Math.Abs(longitude);
            Double longitudeMinutes = (Math.Abs(longitude) - longitudeDegress) * 60;
            String longitudeDirection = (longitude < 0) ? "W" : "E";
            String longitudeDDM = $"{longitudeDegress:000}{longitudeMinutes:00.00}{longitudeDirection}";
            Double altitudeFeet = (altitude * 3.28084);
            return $"{fromCallSign}-{fromSSID}>{toCallSign}:/{this.Time:HHmmss}h{latitudeDDM}{symbolTable}{longitudeDDM}{symbol}/A={altitudeFeet:000000}";
        }
        ...
        public String Comment
		{
            get
			{
                return $"DogStatus:{this.DogStatus} Batt:{this.Battery}/4 ID:{this.CollarId / 256}-{this.CollarId % 256}";
			}
		}
		public TrackedAsset()
        { }

        public TrackedAsset(byte[] buffer)
        {
            int offset = 0;
